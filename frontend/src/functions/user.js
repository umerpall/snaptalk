...
export const removeFromSearchHistory = async (searchUser, token) => {
  try {
    const { data } = await axios.put(
      `${process.env.REACT_APP_BACKEND_URL}/removeFromSearchHistory`,
      { searchUser },
      {
        headers: {
          Authorization: `Bearer ${token}`,
...
export const sendFeedback = async (token) => {
  try {
    const { data } = await axios.get(
      `${process.env.REACT_APP_BACKEND_URL}/getFriendsPageInfos`,
      {
